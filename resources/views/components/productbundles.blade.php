					...
					<div class="w-1/3" v-if="index == 0 && data.amMostviewedBundlePacks.main_product">
						<div class="px-1 my-1">
							<div class="w-full bg-white rounded hover:shadow group relative">
								<div class="block">
									<img v-if="data.amMostviewedBundlePacks.main_product.thumbnail" :src="'/storage/resizes/200/catalog/product' + data.amMostviewedBundlePacks.main_product.thumbnail" class="object-contain rounded-t h-48 w-full mb-3" :alt="data.amMostviewedBundlePacks.main_product.name" loading="lazy" />
									...
									<div class="px-2">
										...
										<div>
											...
											<x-rapidez::select
            									...
            									>
            									<option disabled selected hidden :value="undefined">@lang('Select') {{ $code->label}}</option>
						...
						<div class="w-1/3 border border-gray-200 hover:shadow" v-for="(product, index) in bundle.items" :key="index">
							<div class="px-1 my-1">
								<div class="w-full bg-white rounded group relative">
									<div class="block">
										<img v-if="false" :src="product.product.thumbnail.url" class="object-contain rounded-t h-48 w-full mb-3" :alt="product.product.name" loading="lazy" />
										<x-rapidez::no-image v-else class="rounded-t h-48 mb-3"/>
							...
							<div class="w-full bg-white rounded group relative h-full">
								<div class="block h-full">
									<div class="px-2 flex flex-col justify-between h-full">
										...
										<span class="font-extrabold text-pink text-2xl mb-3">
											@{{ price | price }}
										</span>
